...
### Reproducibility
#### Running the app
Using <b>Docker</b> for containerization, the app comprises of 3 services, each scaffolded by docker into a single app. These serviecs are (as stated in the docker-compose yml):
...
| Chainlit | 8000 | Chat interface |
| Grafana | 3000 | Monitoring (faithfulness, relevance, input/output tokens) |

- Start by running Kestra
  - login into kestra (as provided in the docker yml file)
  - copy the flows from the `kestra/flows` folder into kestra.  
  - Execute the `yc_oss_companies_by_tag` flow; this will trigger other flows as described in the ingestion pipeline below


#### Technologies
- Container :- Docker
- Chat Interface - Chainlit
- Orchestration :- Kestra, dlt
- DB :- Postgres, pgvector, tsvector
- Monitoring :- Pydantic Logfire, Grafana
- Closed source and Open source LLM :- perfomance difference between *llama3.2* and *gpt-5.4-mini* are investigated in the [ventures notebook](./notebooks/ventures.ipynb)
...
```
`yc_oss_to_sample_records` only fires once **both** `yc_oss_to_fulltext` and `yc_oss_to_embeddings` have succeeded (a multi-flow `preconditions` trigger), since it joins their outputs.
...
The notebook also investigates performance of closed (gpt-5.4-mini) vs open model (llama3.2). The chat interface uses *gpt-5.4-mini* as the default model.

The notebook covers generation of ground truths, and checking of Hit rate and MRR using the two different models. In the process of evaluation, we generate ground truths from the sample (1/10th) of the full dataset of ~5k records. Using this sample, five questions are generated per record for ground truths

As summarised under _Hit Rate/MRR Interpretation_ section in the notebook, below are the findings
...
### LLM evaluation
Using RAGAs to evaluate results, looking majorly into Faithfulness and Answer relevance - this guardrails for hallucinations and checking answer/context fit. 

We are also tracking input/output tokens and user feedback on each query. The results from this are summarised in the [monitoring](#monitoring) section 
...
The app also uses **Grafana** to graph from the llm_evaluations table. We are graphing on queries over time, relevance, faithfulness, input/output tokens and user feedback
